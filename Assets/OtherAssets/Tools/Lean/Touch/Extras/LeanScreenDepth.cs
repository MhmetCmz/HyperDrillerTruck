...
namespace Lean.Touch
{
	...
	public struct LeanScreenDepth
	{
		...
		private static readonly RaycastHit[] rcHits = new RaycastHit[128];
		...
		public bool TryConvert(ref Vector3 position, Vector2 screenPoint, GameObject gameObject = null, Transform ignore = null)
		{
			...
			if (camera != null)
			{
				switch (Conversion)
				{
					...
					{
						var ray       = camera.ScreenPointToRay(screenPoint);
						var rcHitCount  = Physics.RaycastNonAlloc(ray, rcHits, float.PositiveInfinity, Layers);
						var bestPoint = default(Vector3);
						var bestDist  = float.PositiveInfinity;

						for (var i = rcHitCount - 1; i >= 0; i--)
						{
							var rcHit         = rcHits[i];
							var rcHitDistance = rcHit.distance;

							if (rcHitDistance < bestDist && IsChildOf(rcHit.transform, ignore) == false)
							{
								bestPoint = rcHit.point + rcHit.normal * Distance;
								bestDist  = rcHitDistance;

								LastWorldNormal = rcHit.normal;
							}
						}
					...
					case ConversionType.PlaneIntercept:
					{
						...
						if (plane != null)
						{
							...
							if (plane.TryRaycast(ray, ref rcHit, Distance) == true)
							{
								position = rcHit;
			...
			EditorGUI.indentLevel++;
			{
				...
				switch (conversion)
				{
					...
					case LeanScreenDepth.ConversionType.PhysicsRaycast:
					{
						...
						GUI.color = color;
						DrawProperty(ref rect, property, label, "Distance", "Offset", "The world space offset from the raycast rcHit point.");
					}
					break;
					...
					case LeanScreenDepth.ConversionType.PlaneIntercept:
					{
						...
						DrawProperty(ref rect, property, label, "Distance", "Offset", "The world space offset from the intercept rcHit point.");
					}
					break;
