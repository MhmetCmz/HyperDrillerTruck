...
namespace Lean.Touch
{
	...
	public class LeanDragColorMesh : MonoBehaviour
	{
		...
		[Tooltip("The color you want to paint the rcHit triangles")]
		...
		private void Paint(LeanFinger finger)
		{
			...
			if (cachedMeshFilter.sharedMesh != null)
			{
				...
				// Raycast under the finger and paint the rcHit triangle
				var rcHit = default(RaycastHit);

				if (Physics.Raycast(finger.GetRay(Camera), out rcHit) == true)
				{
					if (rcHit.collider.gameObject == gameObject)
					{
						var index = rcHit.triangleIndex * 3;
